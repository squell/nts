...
void add_encrypted_server_hdr(unsigned char *buffer, unsigned char **p_ptr, struct NTS nts, const char *cookie) {
        ...
        unsigned char *pt = p;
        p += 16;
        encode_record_raw(&p, 0x0104, "5678____", 8), p-=4;
        encode_record_raw(&p, 0x0204, cookie, strlen(cookie)+4), p-=4;
        *p_ptr = p;
...
static void test_ntp_field_decoding(void) {
        ...
        encode_record_raw(&p, 0x0104, "1234____", 8), p-=4;
        ...
        assert(strcmp((char*)rcpt.new_cookie.data, cookie) == 0);
